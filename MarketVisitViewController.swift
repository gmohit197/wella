//
//  MarketVisitViewController.swift
//  Wella.2
//
...
//MARK:- BEGIN
class MarketVisitViewController: ExecuteApi , CLLocationManagerDelegate ,  UITableViewDelegate, UITableViewDataSource  , UITextViewDelegate {
...
//            self.setList(trainingtypeStr: "Market Visit")
        }
    }
    ...
    override func deletePostSuccess(){
        if(self.validateNetwork()){
            self.postHotDayData(trainingtype: "Market Visit" )
        }
    }
    ...
    func checkList(saloncodeStr : String?) -> Bool
    {
        ...
        self.hotdayTable.isHidden = false
        var stmt4:OpaquePointer?
        let queryString = "SELECT saloncode, salonname FROM hotdaymarketvisit where  saloncode = '" + saloncodeStr! + "'   and trainingtype = 'Market Visit' "
        print("setList"+queryString)
        if sqlite3_prepare_v2(DatabaseConnection.dbs, queryString, -1, &stmt4, nil) != SQLITE_OK{
            let errmsg = String(cString: sqlite3_errmsg(DatabaseConnection.dbs)!)
            print("error preparing get: \(errmsg)")
    ...
    @IBAction func OKBtnclick(_ sender : UIButton){
        if !(remarkText.text! == "" || remarkText.text! == "Enter Remark........" ){
            if(self.validateNetwork()){
                self.updatehotdaymarketvisitremark(remark: remarkText.text!,trainingType: "Market Visit")
                self.remarkView.isHidden = true
                saveBtn.isEnabled = false
                self.postDeleteData(ActivityType: "Market Visit")
            }
        }
        else{
