...
class MyworkshopViewController: ExecuteApi, UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate, UITextFieldDelegate {
    ...
    var ids: [String]!
    @IBOutlet weak var trininglocation: UITextField!
    ...
    override func viewDidLoad() {
        ...
        trininglocation.attributedPlaceholder = NSAttributedString(string: "Training Location",attributes: [NSAttributedString.Key.foregroundColor: UIColor.darkGray])
    ...
    @objc func handleLongPress(_ gestureRecognizer: UILongPressGestureRecognizer){
        ...
        self.textFieldDidBeginEditing(textField: remark)
        self.textFieldDidBeginEditing(textField: trininglocation)
    }
...
//                    }}}
    ...
    @IBAction func save(_ sender: Any) {
        if Approveview.isHidden == true && validation()
        {
    ...
    @IBAction func okbtn(_ sender: UIButton) {
            ...
            if self.noofmodel.text == "" || self.remark.text == ""
            {
                self.showtoast(controller: self, message: "Fields can't be empty", seconds: 1.5)
            }
            else
            {
                self.updatemyworkshop(id: result, model: self.noofmodel.text!, remark: self.remark.text!)
                self.checknet()
                if AppDelegate.ntwrk > 0
                {
                self.uploaddata()
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.55) {
                    self.performSegue(withIdentifier: "okbtnsegue", sender: (Any).self)
                }
                } else
                {
                    self.showtoast(controller: self, message: "Please check Your Internet Connection", seconds: 1.5)
        ...
        DispatchQueue.main.async {
            while(sqlite3_step(stmt1) == SQLITE_ROW){
                ...
                let parameters: [String: AnyObject] = [
                    ...
                    "TRAININGTYPE" : "Workshop" as AnyObject,
            ...
            Alamofire.request(request).responseJSON { response in
                ...
                case .success:
                    ...
                    self.insertsynclog(tablename: "Post Workshop", status: "success", post: "0")
                    break
                    
                case .failure:
                    self.insertsynclog(tablename: "Post Workshop", status: "Failure", post: "0")
    ...
    @IBAction func addbtn(_ sender: UIButton) {
            ...
            {
                var trainingloctaionstr = self.trininglocation.text!
                
                if trainingloctaionstr  == ""
                {
                    trainingloctaionstr = self.idstr!
                }
                //formatter.dateFormat = "ddMMyyyyHHmmss"
                let datetime: String?
                ...
                if validation()
                {
                    self.InsertWorkshop(date: self.inputTextField.text!, trainingcode: self.training, traininglocations: trainingloctaionstr, branchcode: self.idstr, saloncodes: self.selectedsalon, salonname: self.salonname, sellercodes: self.selectedpsr, addresses: self.address.text!, noofstylisttrainedstr: "1", post: "0", modelno: self.noofmodel.text!, remark: self.remark.text!, id: result, Lat: "0.000000000000", longi: "0.000000000000", datetime: datetime, salonremark: self.stylistRemark.text!, stylistname: self.stylistName.text!, stylistcontact: self.stylistcontact.text!)
                    
//                print("\(datetime)  \(result)    \(String(describing: self.salonname!))          (self.noofstylist.text!)")
    ...
    func setlist(){
        ...
        while(sqlite3_step(stmt1) == SQLITE_ROW){
            ...
            self.myworkshopdata.append(workshoplistdata(sno: salon, salon: stylistname, stylist: stylistcontact))
               ...
               // print("\(salon)   \(noofstylist)   \(datetime)")
        }
